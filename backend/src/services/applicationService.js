...
async function saveApplicationAnswers(supabase, applicationId, answers) {
  ...
  const rows = answers
    ...
    .map((a) => ({
      ...
      answer_text:    a.answerType === "text"  ? (toOptionalText(a.answer) ?? null) : null,
      answer_bool:    a.answerType === "yesno" ? (typeof a.answer === "boolean" ? a.answer : null) : null,
    }));
...
async function createApplication({ payload, jwt }) {
  ...
  if (!selectedResumeUrl) {
    const err = new Error("Resume selection is required"); err.status = 400; throw err;
  }
  if (!jdConfirmed) {
    const err = new Error("JD confirmation is required"); err.status = 400; throw err;
  }

  const { data: profile, error: profileError } = await supabase
    .from("profiles").select("is_eligible, eligible_until, application_quota")
    .eq("id", payload.studentId).maybeSingle();
  if (profileError) throw profileError;

  const isEligibleForWindow = profile?.is_eligible === true;
  const eligibleUntil       = profile?.eligible_until ? new Date(profile.eligible_until) : null;

  if (isEligibleForWindow) {
    if (!eligibleUntil || Number.isNaN(eligibleUntil.getTime()) || new Date() > eligibleUntil) {
      const err = new Error("Eligibility window has expired"); err.status = 403; throw err;
    }
  }

  const { data: alreadyApplied, error: alreadyAppliedError } = await supabase
    .from("applications").select("id")
    .eq("student_id", payload.studentId).eq("job_id", payload.jobId).maybeSingle();
  if (alreadyAppliedError) throw alreadyAppliedError;
  if (alreadyApplied?.id) {
    const err = new Error("Already applied for this job"); err.status = 409; throw err;
  }
  ...
  if (!isEligibleForWindow) {
    ...
    if (!Number.isFinite(currentQuota) || currentQuota <= 0) {
      const err = new Error("Application quota exhausted"); err.status = 403; throw err;
    }
    const { data: quotaUpdated, error: quotaUpdateError } = await supabase
      .from("profiles")
      .update({ application_quota: currentQuota - 1, updated_at: new Date().toISOString() })
      .eq("id", payload.studentId).eq("application_quota", currentQuota)
      .select("application_quota").maybeSingle();
    if (quotaUpdateError) throw quotaUpdateError;
    if (!quotaUpdated) {
      const err = new Error("Application quota update conflict. Please try again."); err.status = 409; throw err;
    }
    reservedQuota = currentQuota;
  }

  const { data: resumeRows, error: resumeError } = await supabase
    .from("resumes").select("id, file_url").eq("user_id", payload.studentId);
  if (resumeError) throw resumeError;
  if ((resumeRows || []).length > MAX_RESUMES_PER_STUDENT) {
    const err = new Error(`Maximum ${MAX_RESUMES_PER_STUDENT} resumes are allowed`); err.status = 400; throw err;
  }
  const selectedResume = (resumeRows || []).find((r) => r.file_url === selectedResumeUrl);
  if (!selectedResume) {
    const err = new Error("Selected resume is invalid"); err.status = 400; throw err;
  }
  ...
  try {
    ...
    if (reservedQuota !== null) {
      await supabase.from("profiles")
        ...
        .eq("id", payload.studentId).eq("application_quota", reservedQuota - 1);
    }
    throw insertError;
  }
    ...
    const { error: profileUpdateError } = await supabase
      ...
      .update({
        is_currently_working:
          typeof payload.isCurrentlyWorking === "boolean" ? payload.isCurrentlyWorking : null,
...
async function listAllApplications({ actor, jwt }) {
  const supabase = getClient(jwt);
  const { data, error } = await supabase
    .from("applications").select(APPLICATIONS_ADMIN_SELECT)
...
async function getStudentAnalytics({ studentId, jwt }) {
  ...
  const [
    ...
    supabase.from("jobs").select("id", { count: "exact", head: true })
      .eq("status", "active").gte("valid_till", today),
  ...
  const statusCounts = APPLICATION_STATUSES.reduce((acc, s) => { acc[s] = 0; return acc; }, {});
  (apps || []).forEach((a) => {
  ...
  const eligibility = profile?.is_eligible
    ...
    : { type: "quota", remainingApplications: Number(profile?.application_quota ?? 0) };
