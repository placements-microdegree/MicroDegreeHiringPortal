  ...
  const rows = [
    {
      ...
      value:
        form.isCurrentlyWorking === true
          ? "Yes"
          : form.isCurrentlyWorking === false
            ? "No"
            : "—",
    },
    ...
    { label: "Expected CTC", value: form.expectedCTC || "—" },
    {
      ...
      value:
        form.handsOnPrimarySkills === true
          ? "Yes"
          : form.handsOnPrimarySkills === false
            ? "No"
            : "—",
    },
    {
      ...
      value:
        form.workModeMatch === true
          ? "Yes"
          : form.workModeMatch === false
            ? "No"
            : "—",
    },
    {
      ...
      value:
        form.interviewModeAvailable === true
          ? "Yes"
          : form.interviewModeAvailable === false
            ? "No"
            : "—",
    },
  ].filter(Boolean);
  ...
  const buildFormFromSources = useCallback(
    ({ previousApplication, resumeRows }) => {
      ...
      const hasPreviousResume = safeResumes.some(
        (r) => r?.file_url === previousResumeUrl,
      );
      ...
      return {
        ...
        relevantExperience:
          ...
          "",
        studentConcern:
          previousApplication?.student_concern ||
          previousApplication?.studentConcern ||
          "",
  ...
  const submitApplication = async (saveForFuture) => {
    ...
    try {
      ...
      const payload = {
        ...
        studentConcern: form.studentConcern,
        ...
        >
          ...
          <section className="space-y-4 lg:grid lg:grid-cols-2 lg:gap-4 lg:space-y-0">
            <QuestionToggle
              ...
              onChange={(value) => {
                setTouched((p) => ({ ...p, isCurrentlyWorking: true }));
                update({
            ...
            {form.isCurrentlyWorking ? (
              <Input
                ...
                value={form.noticePeriod}
                onChange={(e) => {
                  setTouched((p) => ({ ...p, noticePeriod: true }));
                  update({ noticePeriod: e.target.value });
                }}
                placeholder="e.g. 30 days"
                error={
                  show("noticePeriod") && errors.noticePeriod ? "Required" : ""
                }
              />
            ) : (
            ...
            <div>
              <div
                className={`mb-1.5 text-sm font-medium ${show("totalExperience") && errors.totalExperience ? "text-red-600" : "text-slate-700"}`}
              >
                Total Experience
              ...
              <div
                className={`rounded-xl border px-4 py-3 ${show("totalExperience") && errors.totalExperience ? "border-red-400 bg-red-50" : "border-slate-200 bg-slate-50"}`}
              >
                ...
                <input
                  ...
                  onChange={(e) => {
                    setTouched((p) => ({ ...p, totalExperience: true }));
            ...
            <Input
              ...
              value={form.relevantExperience}
              onChange={(e) => {
                setTouched((p) => ({ ...p, relevantExperience: true }));
                update({ relevantExperience: e.target.value });
              }}
              placeholder="e.g. 2 years in React"
              error={
                show("relevantExperience") && errors.relevantExperience
                  ? "Required"
                  : ""
              }
            />
            ...
            <QuestionToggle
              ...
              onChange={(value) => {
                ...
                update({ handsOnPrimarySkills: value });
              }}
              error={
                show("handsOnPrimarySkills") && errors.handsOnPrimarySkills
              }
            />
            ...
            <QuestionToggle
              label={
                jobWorkMode
                  ? `Are you comfortable with ${jobWorkMode} work mode?`
                  : "Work mode preference match?"
              }
              sublabel={
                jobWorkMode ? `This role requires ${jobWorkMode}` : undefined
              }
              value={form.workModeMatch}
              onChange={(value) => {
                setTouched((p) => ({ ...p, workModeMatch: true }));
                update({ workModeMatch: value });
              }}
            ...
            <QuestionToggle
              label={
                interviewModeDisplay
                  ? `Are you available for a ${interviewModeDisplay} interview?`
              ...
              onChange={(value) => {
                setTouched((p) => ({ ...p, interviewModeAvailable: true }));
                update({ interviewModeAvailable: value });
              }}
              error={
                show("interviewModeAvailable") && errors.interviewModeAvailable
              }
            />
            ...
            <Input
              ...
              onChange={(e) => {
                setTouched((p) => ({ ...p, currentCTC: true }));
                update({ currentCTC: e.target.value });
              }}
            ...
            <Input
              ...
              onChange={(e) => {
                setTouched((p) => ({ ...p, expectedCTC: true }));
                update({ expectedCTC: e.target.value });
              }}
              placeholder="e.g. 6.5 LPA"
              error={
                show("expectedCTC") && errors.expectedCTC ? "Required" : ""
              }
            />
          ...
          <section className="rounded-xl border border-slate-200 bg-slate-50 p-3 sm:p-4">
            <label className="block">
              ...
              <textarea
                rows={3}
                value={form.studentConcern}
                onChange={(e) => update({ studentConcern: e.target.value })}
                placeholder="In case of any concerns, feel free to write to us."
                className="w-full resize-y rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 outline-none focus:border-primary"
              />
            </label>
          </section>
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <div
                  className={`text-sm font-semibold ${show("selectedResumeUrl") && errors.selectedResumeUrl ? "text-red-700" : "text-slate-900"}`}
                >
                  Resume Selector
              ...
              <label
                className={`inline-flex cursor-pointer items-center gap-2 rounded-xl border px-3 py-2 text-xs font-semibold transition ${resumes.length >= MAX_RESUMES || uploading ? "cursor-not-allowed border-slate-200 text-slate-400" : "border-slate-300 text-slate-700 hover:border-primary hover:text-primary"}`}
              >
